	...
	def backprop_mse(self, inputs, lab, alpha = 0.001):
		...
		for count, layer in enumerate(list(reversed(self.layers))):
			...
			if count != len(self.layers) - 1:
				...
				if(layer.l2==0 && layer.l1==0):
					delta_w = np.matmul((self.layers[count - 1].out).transpose(), delta)
				elif(layer.l1 ==0 && layer.l2!=0):
					delta_w = np.matmul((self.layers[count - 1].out).transpose(), delta) + layer.l2*layer.weight
				elif(layer.l2==0 && layer.l1!=0):
					delta_w = np.matmul((self.layers[count - 1].out).transpose(), delta)
					id_matrix = layer.weight + 1e-10
					reg_matrix = abs(id_matrix)/id_matrix
					delta_w = delta_w + layer.l1 * id_matrix
				...
				if(layer.l2==0 && layer.l1==0):
					...
					delta_w = np.matmul((inputs).transpose(), delta) + layer.l2*layer.weight
				elif(layer.l2==0 && layer.l1!=0):
					delta_w = np.matmul((inputs).transpose(), delta)
					id_matrix = layer.weight + 1e-10
					reg_matrix = abs(id_matrix)/id_matrix
...
class Layer:
	...
	def __init__(self, in_sz, out_sz, activation=None, bn=False, epsilon=1e-8, l1=0, l2=0):
		self.in_size = in_sz
